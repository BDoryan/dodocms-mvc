<div class="relative group w-full bg-gray-700 px-3 py-2 border border-gray-500 rounded-md text-white outline-none focus:border-gray-400 mb-1 h-[300px] flex flex-col overflow-hidden">
    <div class="opacity-0 group-hover:opacity-100 group-hover:bg-gray-400/[.20] absolute top-0 bottom-0 left-0 right-0 flex flex-col">
        <div class="m-auto flex flex-col text-center text-gray-300 text-opacity-75">
            <i class="fa-solid fa-plus-circle text-4xl"></i>
            <span class="mt-2">
            Appliquer une resource pour cet élément.
        </span>
        </div>
    </div>
    <div class="group-hover:opacity-0 m-auto text-gray-300 text-opacity-75">
        Vous avez défini aucune ressource pour cet élément.
    </div>
</div>