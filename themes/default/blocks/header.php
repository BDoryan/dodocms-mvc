<div id="home" class="position-relative welcome">
    <div class="d-flex background">
        <img class="background-image" src="https://www.doryanbessiere.fr/assets/imgs/background.jpg" alt="fond">
        <div class="position-absolute top-100 start-50 translate-middle row g-2 container justify-content-center ">
            <a href="#experiences" class="col-auto text-center text-decoration-none text-white">
                <div class="info-badge py-3 px-5 rounded-5 bg-dark shadow h-100 d-flex">
                    <h5 class="mt-auto mb-auto"><span class="text-primary">7 années</span> en autodidacte</h5>
                </div>
            </a>
            <a href="#projects" class="col-auto text-center text-decoration-none text-white">
                <div class="info-badge py-3 px-5 rounded-5 bg-dark shadow h-100 d-flex">
                    <h5 class="mt-auto mb-auto"><span class="text-primary">14 projets</span> réalisés</h5>
                </div>
            </a>
        </div>
    </div>
    <div class="position-absolute top-50 start-50 translate-middle text-white text-center">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <h1 class="fw-bolder animate__animated animate__rubberBand ignore" editable="title">Doryan BESSIERE</h1>
            </div>
            <div class="col-md-auto">
                <h3 editable="subtitle" class="fw-bold mb-5 typewriter"><span class="text-uppercase">Développeur full-stack</span>
                </h3>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-auto d-flex"><i class="fs-1 mt-auto mb-auto fa-brands fa-java"></i></div>
            <div class="col-auto d-flex"><i class="fs-1 mt-auto mb-auto bi bi-bootstrap-fill mt-auto mb-auto"></i>
            </div>
            <div class="col-auto d-flex"><i class="fs-1 mt-auto mb-auto fa-brands fa-php"></i></div>
            <div class="col-auto d-flex"><i class="fs-1 mt-auto mb-auto fa-brands fa-react"></i></div>
            <div class="col-auto d-flex"><i class="fs-1 mt-auto mb-auto bi bi-database mt-auto mb-auto"></i></div>
        </div>
    </div>
</div>